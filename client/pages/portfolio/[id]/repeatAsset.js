...
import { IconArrowLeft, IconPlus, IconAlertCircle, IconChartLine } from '@tabler/icons-react';
...
export default function RepeatAssetPage() {
  ...
  // Pre-fill the symbol from the query string
  useEffect(() => {
    if (router.isReady && symbol && stockOptions.length > 0) {
      const matched = stockOptions.find(opt => opt.value === symbol.replace(/\.BSE$/, ''));
      if (matched) {
        setFormData(prev => ({ ...prev, symbol: matched.value }));
      }
    }
  }, [router.isReady, symbol, stockOptions]);
